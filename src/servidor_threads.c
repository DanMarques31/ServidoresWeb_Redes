...
int main() {
    ...
    // Declaração e inicialização da fila de tarefas
    FilaTarefas fila;
    inicializar_fila(&fila, 100);  // Tamanho da fila

    // Declaração e Inicialização das threads
    ...
    while (1) {
        ...
        if (sscanf(buffer, "GET %s", diretorio) == 1) {
            ...
            strncpy(tarefa.diretorio, diretorio + 1, TAMANHO_BUFFER - 1);
            ...
            enviar_resposta(socket_cliente, mensagem, strlen(mensagem), "text/plain"); // Envia a resposta de erro
            close(socket_cliente);
        }
    }
